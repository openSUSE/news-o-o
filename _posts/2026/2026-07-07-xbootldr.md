...
`XBOOTLDR` is a new partition that can live anywhere in the disk.  The `ESP` has some limitations in that regard, and usually is the first partition in the system.  If present, `systemd-boot` will look for the menu entries and the kernel / `initrd`s in there, freeing the `ESP` of that responsibility.
...
For more information, see the `Further documentation` section for links with a more detailed description.
